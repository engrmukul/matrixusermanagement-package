...
class ModuleRepository extends BaseRepository implements ModuleContract
{
    ...
    public function moduleList(string $order = 'id', string $sort = 'desc', array $columns = ['*'])
    {
        ...
        return Datatables::of($query)
            ->addColumn('action', function ($row) {
                ...
                $actions .= '<a class="btn btn-primary btn-xs float-left mr-1" href="' . route('modules.edit', [$row->id]) . '" title="Module Edit"><i class="fa fa-pencil"></i>' . trans("common.edit") . '</a>';
    ...
    {
        $module = $this->findById($params['id']);
