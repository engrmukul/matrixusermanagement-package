...
use Mukul\Matrixusermanagement\Contracts\UserRoleContract;
...
class UserRoleRepository extends BaseRepository implements UserRoleContract
{
     ...
     * @return mixed
     */
    public function userRoleList(string $order = 'id', string $sort = 'desc', array $columns = ['*'])
    {
        ...
        return Datatables::of($query)
            ->addColumn('action', function ($row) {
                ...
                $actions .= '<a class="btn btn-primary btn-xs float-left mr-1" href="' . route('user-roles.edit', [$row->id]) . '" title="UserRole Edit"><i class="fa fa-pencil"></i>' . trans("common.edit") . '</a>';

                $actions .= '
                    <form action="' . route('user-roles.destroy', [$row->id]) . '" method="POST">
     ...
     * @return mixed
     */
    public function findUserRoleById(int $id)
    {
        try {
     ...
     */
    public function createUserRole(array $params)
    {
        try {
     ...
     * @return mixed
     */
    public function updateUserRole(array $params)
    {
        $userRole = $this->findById($params['id']);
     ...
     * @return bool|mixed
     */
    public function deleteUserRole($id, array $params)
    {
        $userRole = $this->findUserRoleById($id);
